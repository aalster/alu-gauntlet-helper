...
from alu_gauntlet_helper.screen_recognition.regions import (
    BEFORE_RACE_MIN_WHITE_FRAC, BEFORE_RACE_SLOTS, BEFORE_RACE_TRACK,
    BEFORE_RACE_WHITE_THRESHOLD,
)
...
class BeforeRaceExtractor(ScreenExtractor):
    ...
    поточний номер яскраво-білий. Дані суперника (його авто і "TIME TO BEAT")
    та авто гравця з цього екрана НЕ беремо — повертаємо лише номер гонки і трек,
    рештою полів сесія наповниться з екранів результату.
    """

    name = "before_race"

    def __init__(self, track_resolver: TrackResolver):
    ...
    def extract(self, img: np.ndarray) -> list[RaceCapture]:
        ...
        track_text = ocr.read_name(BEFORE_RACE_TRACK.crop(img))
